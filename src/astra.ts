...
if (!fs.existsSync(path.join(os.homedir(), '.astra', 'versions'))) {
        fs.mkdirSync(path.join(os.homedir(), ".astra", "versions"), { recursive: true })
}
...
cli.
command("versions", "Show available versions of Node.js", () => {}, async () => {
    (await import("./versions.js")).default();
});

cli.
command("install [ver]", "Preinstall Node.js version (format node_{version eg. v22.14.0}-{platform, likely win}-{arch eg. x64, x86, arm64}", (y) => {
    y.positional("ver", {
        describe: "Node.js version to install",
        type: "string"
    })
}, async (argv) => {
    (await import("./install.js")).default(argv as any)
})

cli.
command("build [entry]", "Build your project", (y) => {
    y.positional("entry", {
    ...
    y.option("node", { 
        describe: "Node.js version to use (in astra format eg. node_v22.14.0-win-x64)",
        type: "string",
        alias: "n",
    })
    y.option("disShasumCheck", {
        describe: "Disable node.exe shasum check (only use if you know what you're doing!)",
        type: "boolean"
    });
}, async (argv) => {
    (await import("./build.js")).default(argv as any);
})
