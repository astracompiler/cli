...
cli.command(
	...
	() => {},
	async () => {
		(await import("./versions.ts")).default();
	},
);
...
cli.command(
	...
	async (argv) => {
		(await import("./install.ts")).default(argv as unknown as { ver: string });
	},
);
...
cli.command(
	...
	async (argv) => {
		...
		(await import("./build.ts")).default(argv as unknown as BuildArgs);
	},
);
cli.command(
	"init",
